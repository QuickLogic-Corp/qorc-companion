...
//from lyrat_init.c
extern int get_phrase_length(void);
        ...
        switch (va_dsp_data.dsp_state) {
            case STREAMING:
                switch (event_data.event) {
                    ...
                    case STOP_MIC:
                        _va_dsp_stop_streaming();
                        printf("%s: Event %d STOP_MIC in STREAMING state\n", TAG, event_data.event);
                        break;
                    case MUTE:
                        _va_dsp_mute_mic();
                        printf("%s: Event %d MUTE in STREAMING state\n", TAG, event_data.event);
                        break;
                    case WW:
                    case START_MIC:
                    case UNMUTE:
            ...
            case STOPPED:
                switch (event_data.event) {
                    ...
                    case TAP_TO_TALK:
          //if (va_dsp_data.va_dsp_recognize_cb(get_phrase_length(), WAKEWORD) == 0) {
                    ...
                    case MUTE:
                        _va_dsp_mute_mic();
                        break;
                    case GET_AUDIO:printf("%s: Event %d (GET_AUDIO) unsupported in STOPPED state\n", TAG, event_data.event);break;
                    case STOP_MIC:printf("%s: Event %d (STOP_MIC) unsupported in STOPPED state\n", TAG, event_data.event);break;
                    case UNMUTE:printf("%s: Event %d (UNMUTE) unsupported in STOPPED state\n", TAG, event_data.event);break;
                    default:
...
extern void stopQFAudioInput(void);
    stopQFAudioInput();
...
int va_dsp_tap_to_talk_start()
{
    ...
    //this is called from host task. so it should not block
    BaseType_t result = xQueueSend(va_dsp_data.cmd_queue, &new_event, 5);
    if(result != pdTRUE)  {
        xQueueReset(va_dsp_data.cmd_queue);
        result = xQueueSend(va_dsp_data.cmd_queue, &new_event, 5);
        ESP_LOGE(TAG, "Error timeout sending to va_dsp cmd_queue. Resetting Queue ... ");
...
{
//not good. prints too many
//printf("%s: Playback is starting ...\n", TAG);
    return 0;
}
...
printf("%s : Before lyrat init \n",TAG);

    lyrat_init();
printf("%s : After lyrat init \n",TAG);
