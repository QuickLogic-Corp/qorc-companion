...
int alexa_signin = 0;
void amazon_signin_callback(void *in_ptr)
{
    printf("==== Amazon Sign in Successful === \n");
    alexa_signin = 1;
}
void amazon_signout_callback(void *out_ptr)
{
    printf("==== Amazon Sign out Happened === \n");
    alexa_signin = 0;
}
//This seems to be the only way to check if wifi is connected
...
//use this to reset the processor
void reboot_esp32(void)
{
    /* Just to go to the next line */
    printf("Reboot through SW reset\n");
    va_reset();
    vTaskDelay(2);//wait 20ms
    esp_restart(); //software reboot. never returns
    return;
}
...
void app_main()
{
    ...
    alexa_auth_delegate_init(amazon_signin_callback, amazon_signout_callback);
...
printf("%s :host task created\n", TAG);
    ...
    if (!provisioned) {
        ...
        printf("%s: Already provisioned, starting station\n", TAG);
...
printf("%s : before Alexa config %d \n", TAG,provisioned );
...
int8_t tx_power = 0;
    ...
    printf("%s : Tx Power = %d dbm\n", TAG, (tx_power + 3)/4 ); //since tx_power is in 0.25dbm units
    return;
}
