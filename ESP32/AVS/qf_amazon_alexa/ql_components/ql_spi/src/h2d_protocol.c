...
int h2d_error_count = 0;
/* Internal APIs*/
extern void reboot_esp32(void);
void check_h2d_errors(void)
{
    h2d_error_count++;
    if(h2d_error_count > 10) {
      reboot_esp32();
    }
    return;
}
...
static int start_recv_lock(void) {
   //take the lock
   if (xSemaphoreTake(g_h2d_transmit_lock, pdMS_TO_TICKS(1000)) != pdTRUE) {
...
void h2dRxTaskHandler(void *pParameter){
    ...
    while(!h2dRxTaskStop){
        ...
        case H2DRX_MSG_INTR_RCVD:
        {
            ...
            out_gpio_val = read_gpio_out_val(out_gpio);
#if (DEBUG_H2D_PROTOCOL_2 ==1)
            printf("[H2D Protocol]: QL_INT value = %d\n", out_gpio_val);
#endif
...
#endif                
#if (DEBUG_H2D_PROTOCOL_2 ==1)
                printf("[H2D Protocol]:Transmit from host complete. Received pulse from device\n");
#endif
            }
            else{           // QL_INT is low
                /* This is an event from device.
...
#define PRIORITY_TASK_H2D_RX              (configMAX_PRIORITIES - 8) //(CONFIG_ESP32_PTHREAD_TASK_PRIO_DEFAULT + 2)
#define STACK_SIZE_TASK_H2D_RX            (4*1024)
...
#else
   //after writing into S3 memory, wait for device stop
   while (read_gpio_intr_val(in_gpio)) {
        vTaskDelay(1); // Leave 2 ticks for other process to execute
        //ets_delay_us(10); //wait 10 micro secs
   }
...
//Test is done with LyraT but migrated to DevKitC-VE with WROVER module
#define USE_LYRAT_BOARD 0
#define USE_DEVKITC_VE_BOARD 1
